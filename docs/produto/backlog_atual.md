...
| **2º** | 13 | Ambiente de staging — **blueprint + scaffolding** (`docs/arquitetura/staging.md`, `*/.env.staging.example`) **e infra provisionada**: projeto Railway **`nutrideby-staging`** (separado da prod) com **Postgres+pgvector** e **api-gateway** verdes (domínio `api-gateway-production-7db7.up.railway.app`), variáveis/segredos próprios (Asaas sandbox, `FIELD_ENCRYPTION_KEY` própria), schema Prisma aplicado via pre-deploy. **Bloqueado:** `rag-agent` crasha no boot faltando `ANTHROPIC_API_KEY`/`EMBEDDING_API_KEY` (chaves pagas do usuário) — as mesmas travam o seed TACO do `knowledge_base`. **Pendente:** chaves do rag-agent, seed (TACO + dados sintéticos), frontends Vercel de staging + `CORS_ORIGIN`. Dados: só sintéticos (LGPD). | M | 🟡 backend no ar; rag-agent + seed + frontends pendentes |
